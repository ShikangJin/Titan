...
public class GeoRecommendation implements Recommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		Set<String> favoriteItems = conn.getFavoriteItemIds(userId); // step 1 fatch all the events ids
		
		Set<String> allCategories = new HashSet<>(); //step 2 find all events categories
		for (String item : favoriteItems) {
			allCategories.addAll(conn.getCategories(item)); // db queries
		}
		
		//step 3 find events belong to these categories
		...
		List<Item> filteredItems = new ArrayList<>(); // step 4 filter events that this user has visited
